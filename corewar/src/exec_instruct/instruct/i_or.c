...
static void exec_or(vm_t *vm, process_t *process, params_t *params)
{
...
static int init_or(vm_t *vm, process_t *process, unsigned char indicator)
{
    ...
    exec_or(vm, process, params);
    free(params);
    return size_skip;
}

int i_or(vm_t *vm, __attribute__((unused))champion_t *champion,
    process_t *process)
{
    unsigned char indicator = (unsigned char)get_param(vm, process->coord_pc.x,
        process->coord_pc.y + T_ID, T_INFO);
    int size_skip = init_or(vm, process, indicator);
